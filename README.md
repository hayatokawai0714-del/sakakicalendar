# 出荷・予定管理アプリ（静的版）

## アプリ概要
HTML / CSS / JavaScript だけで動く、出荷・予定管理の試作アプリです。  
サーバーは使わず、データは `localStorage` に保存します。GitHub Pages で公開できます。

主な機能:
- 今日の予定一覧
- 月間カレンダー（日曜始まり、今日強調、日付クリックで当日一覧）
- 出荷 / 予定 / メモの追加・編集・削除
- 出荷先マスタ管理（詳細項目保持）
- 規格・単位のカスタマイズ（追加・編集・削除）

## スポット出荷と定期出荷
### スポット出荷
- 単発の出荷予定
- 指定した1日だけカレンダーに表示
- `localStorage: sakaki_entries_v1` に `type:"shipment", shipmentType:"spot"` として保存

### 定期出荷
- ルール（繰り返し条件）を登録すると、カレンダー表示時に当月分だけ自動展開して表示
- `localStorage: sakaki_recurring_shipments_v1` に保存
- カレンダー上では `出荷 定期` のラベルで表示

注意:
- 定期出荷の「削除」は、その日の1件だけでなく **定期ルール自体の削除** です
- `endDate` が空の場合は終了日なし

## 定期出荷の設定方法
1. 「予定を追加 / 編集」→ 種別を `出荷`
2. `出荷区分` を `定期` に変更
3. 繰り返し種類を選択
   - `毎週`（intervalWeeks=1）
   - `隔週`（intervalWeeks=2 / startDate基準）
   - `毎月指定日`（monthDays）
4. 条件を入力して保存

### 対応している繰り返し条件
- 毎週指定曜日（複数曜日可）
- 隔週指定曜日（startDateを基準に隔週判定）
- 毎月指定日（複数日可、31日が存在しない月は表示しない）
- 期間指定（startDate〜endDate。endDate空欄は無期限）

## ファイル構成
- `index.html`
- `style.css`
- `app.js`

## 使い方
1. `index.html` をブラウザで開く
2. 出荷/予定/メモを保存
3. カレンダーの日付をタップして当日一覧を確認
4. 当日一覧の `編集` / `削除` で変更
5. 出荷先・規格・単位は下部セクションで管理

補足:
- カレンダーは1日あたり最大3件表示、超過分は `他◯件`
- 長文は省略表示
- 住所/電話などは出荷先マスタのデータとして保持し、カレンダーには表示しません

## GitHub Pagesでの公開方法
推奨: `docs/` 公開（開発用ファイルと分離）  
1. GitHub リポジトリの `Settings` → `Pages` を開く  
2. `Source` を `Deploy from a branch` に設定  
3. Branch を `main` / Folder を `/docs` に設定して保存  
4. 公開URLへアクセス  

想定URL:
- `https://hayatokawai0714-del.github.io/sakakicalendar/`

うまく公開されない場合のチェック:
- `docs/index.html` が存在するか
- `docs/.nojekyll` が存在するか
- `docs/index.html` の読み込みが `./app.js` / `./style.css` になっているか（`/app.js` はNGになりやすい）

## Googleスプレッドシート連携手順（GAS API）
1. Googleスプレッドシートを新規作成
2. スプレッドシートから `拡張機能 → Apps Script` を開く
3. `gas/Code.gs` の内容を貼り付ける（既存コードは置き換え）
4. `SPREADSHEET_ID` を自分のスプレッドシートIDに設定
5. `デプロイ → 新しいデプロイ → 種類: ウェブアプリ` でデプロイ
6. 実行ユーザー: 自分
7. アクセスできるユーザー: 運用に合わせて設定（まずはテスト用に広めでもOK）
8. 発行されたWebアプリURLを、アプリの「同期設定」→ `API URL` に設定
9. `updatedBy`（更新者名）も必要なら入力して保存
10. GitHub Pagesで動作確認

### 注意点
- `API URL` 未設定時は localStorage 保存（端末ごとに別データ）
- `API URL` 設定時は Googleスプレッドシート保存（複数人で共有）
- 複数人で使う場合は **同じWebアプリURL** を全員が設定
- スプレッドシートを直接編集する場合は列名（ヘッダー）を変えない

## 今後のTODO
- 複数人共有（認証 + API）
- Googleスプレッドシート連携
- FAX注文書画像アップロード
- OCRによる入力補助
- iPhoneホーム画面ウィジェット風表示
- 定期出荷の例外日（祝日スキップ等）
- 定期出荷の個別発生分だけ削除/変更（ルールは残す）
